...
static inline int32_t get_tensor_value(const Tensor* tensor, int index) {
    if (tensor->dtype.type_enum == OstarNN_FLOAT) {
        return *((float*)(tensor->ptr) + index);
    } else if (tensor->dtype.type_enum == OstarNN_INT) {
        return *((int32_t*)(tensor->ptr) + index);
    } else if (tensor->dtype.type_enum == OstarNN_INT8) {
        return *((int8_t*)(tensor->ptr) + index);
    } else {
        OSTARNN_ASSERT_MSG(0, "unsupport dtype.\n");
    }
    return 0;
}
#endif