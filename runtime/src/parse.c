...
static float half_to_float(const uint16_t x) {
    const ccuint e = (x & 0x7C00) >> 10;      
    const ccuint m = (x & 0x03FF) << 13;       
    const ccuint v = as_uint((float)m) >> 23;  
...
static OstarNNFormat format_from_fbs(ns(Format_enum_t) fbs_format) {
    switch (fbs_format) {
        case ns(Format_NCHW):
            return OstarNN_NCHW;
        case ns(Format_NHWC):
        ...
        case ns(Format_NCHW4):
            return OstarNN_NCHW4;
        case ns(Format_NCHW8):
            return OstarNN_NCHW8;
        case ns(Format_OIHW):
            return OstarNN_OIHW;
        default: {
...
static OstarNNDevice device_from_fbs(ns(Device_enum_t) fbs_device) {
    switch (fbs_device) {
        case ns(Device_BARE_METAL):
            return OstarNN_BARE_METAL;
        case ns(Device_ARM64):
            return OstarNN_ARM64;
        case ns(Device_ARM32):
            return OstarNN_ARM32;
        case ns(Device_ARM64_V82):
            return OstarNN_ARM64_V82;
        case ns(Device_ARM32_V82):
            return OstarNN_ARM32_V82;
        case ns(Device_OPENCL_MALI):
            return OstarNN_OPENCL_MALI;
        default: {
...
static OstarNNStatus alignment_or_alloc_weights(
    ...
    for (int i = 0; i < nr_weight; i++) {
        ...
        int compressed = 0;
        if (length > 0 && weight->dtype.type_enum ==  OstarNN_FLOAT &&
...
OstarNNStatus parse_device_model(
    ...
    model->tensors =  ostarnn_malloc(sizeof(Tensor) * nr_tensor);
    ...
    for (int i = 0; i < nr_tensor; i++) {
        ...
        if (parse_tensor(tensor, fbs_tensor, i) !=  OstarNN_SUCCESS) {
    ...
    model->instructions = ostarnn_malloc(sizeof(Instruction) * nr_instruction);
    ...
    model->inputs = ostarnn_malloc(model->nr_input * sizeof(Tensor*));
    ...
    model->outputs = ostarnn_malloc(model->nr_output * sizeof(Tensor*));
    ...
    for (int i = 0; i < nr_weight_outputs; ++i) {
        ...
        *(model->outputs + i + nr_outputs) = c_model->weights + index;
        ...
        weight_output_ptr->name = ostarnn_malloc(strlen(name) + 1);
...
exit:
    if (model->tensors) {
        ...
        ostarnn_free(model->tensors);
    }
    if (model->instructions) {
        for (int i = 0; i < model->nr_instruction; i++) {
            Instruction* inst = model->instructions + i;
            if (inst->tag == OstarNN_INST_OPR) {
        ...
        ostarnn_free(model->instructions);
    }
    return OstarNN_ERROR_MODEL_PARSE;
}

static inline bool valid_device_check(OstarNNDevice device) {
    if (device == OstarNN_BARE_METAL) {
        return true;
    }
#if defined(__aarch64__)
    else if (device == OstarNN_ARM64) {
        return true;
    }
#elif defined(__arm__)
    else if (device == OstarNN_ARM32) {
        return true;
    }
#endif
    return false;
 ...
 OstarNNStatus parse_model(
    ...
    model->host_dev.device_type =  OstarNN_BARE_METAL;
    OSTARNN_ASSERT(init_device(&(model->host_dev)) ==  OstarNN_SUCCESS);
    ...
    model->weights = ostarnn_malloc(sizeof(Tensor) * nr_weight);
    ...
    for (int i = 0; i < nr_weight; i++) {
        ...
        if (parse_weight(weight, fbs_weight, &(model->host_dev)) !=  OstarNN_SUCCESS) {
    ...
    int nr_valid_device_model = 0;
    {
        ...
        for (int i = 0; i < nr_model; i++) {
            ...
            ns(Device_enum_t) fbs_device = ns(DeviceModel_device(fbs_device_model));
             OstarNNDevice device_enum = device_from_fbs(fbs_device);
        ...
        model->max_tensor_memroy = ostarnn_malloc(sizeof(Memory));
    ...
    LOG_DEBUG(
            ...
            nr_valid_device_model);
    model->device_models =  ostarnn_malloc(sizeof(DeviceModel*) * nr_valid_device_model);
    ...
    for (int i = 0, valid_model_idx = 0; i < nr_model; i++) {
        ...
        DeviceModel* dev_model = ostarnn_malloc(sizeof(DeviceModel));
        LOG_DEBUG(
        ...
        if (init_device(&dev_model->device) !=  OstarNN_SUCCESS) {
            goto exit;
        }
        dev_model->opt = create_runtime_opt(&dev_model->device);
        if (parse_device_model(dev_model, model, fbs_device_model) !=  OstarNN_SUCCESS) {
...
exit:
    if (model->name) {
         ostarnn_free(model->name);
    }
    if (model->model_ptr) {
        ostarnn_free(model->model_ptr);
    }
    if (model->is_own_tensor_memory && model->max_tensor_memroy) {
        ostarnn_free(model->max_tensor_memroy);
    }
    if (model->weights) {
        for (int i = 0; i < model->nr_origin_weight; i++) {
            ...
            if (weight->name)
                ostarnn_free(weight->name);
        }
        ostarnn_free(model->weights);
    }
    if (model->device_models) {
        for (int i = 0; i < model->nr_device_model; ++i) {
            ostarnn_free(model->device_models[i]);
        }
        ostarnn_free(model->device_models);
    }
    return  OstarNN_ERROR_MODEL_PARSE;
}